...
scatter3(targetTrain(:,1),targetTrain(:,2),targetTrain(:,3),15,[0 0 1],'filled');
hold on;
...
scatter3(targetTest(:,1),targetTest(:,2),targetTest(:,3),15,[0 0 1],'filled');
hold on;
...
xlabel('Target Output');
ylabel('Model Output');
saveas(h,'scatter test.png');


%%scatter plot for val data

 h =  figure;
targetVal = [valData valDataTarget];
modelOutput = net(valData');

scatter3(targetVal(:,1),targetVal(:,2),targetVal(:,3),15,[0 0 1],'filled');
hold on;
...
figure
surf(p1,p2,z2);
title(['output of second hidden layer node' num2str(j)]);
colormap autumn
end
...
%    title('output of output node');
%      colormap winter
  %% plot of target Output on the Approximated surface for test data   
     ...
     hold on;
     scatter3(targetTest(:,1),targetTest(:,2),targetTest(:,3),30,[0 1 0],'filled');
     title('Target Output of Test Data on Approximated Surface');
     saveas(h,'ApproxSurfaceTest.fig')
     colormap copper
     color bar
     
      %% plot of target Output on the Approximated surface for train data 
     xMin = min(trainData(:,1));
    xMax = max(trainData(:,1));
    ...
    yMax = max(trainData(:,2));
     ...
     XgridNew = reshape(Xgrid,[size(Xgrid,1)*size(Xgrid,2),1]);
     YgridNew = reshape(Ygrid,[size(Ygrid,1)*size(Ygrid,2),1]);
     trainDataGrid = [XgridNew YgridNew];
     modelGrid = net(trainDataGrid');
     modelGridNew = reshape(modelGrid,[size(Xgrid,1),size(Ygrid,1)]);
    h =  figure;
     surf(Xgrid,Ygrid,modelGridNew);
     hold on;
     scatter3(targetTrain(:,1),targetTrain(:,2),targetTrain(:,3),30,[0 1 0],'filled');
     title('Target Output of Train on Approximated Surface');
     saveas(h,'ApproxSurfaceTrain.fig')
     colormap copper
     color bar
     
     
      %% plot of target Output on the Approximated surface for validation data 
     xMin = min(valData(:,1));
    xMax = max(valData(:,1));
    ...
    yMax = max(valData(:,2));
     ...
     XgridNew = reshape(Xgrid,[size(Xgrid,1)*size(Xgrid,2),1]);
     YgridNew = reshape(Ygrid,[size(Ygrid,1)*size(Ygrid,2),1]);
     valDataGrid = [XgridNew YgridNew];
     modelGrid = net(valDataGrid');
     modelGridNew = reshape(modelGrid,[size(Xgrid,1),size(Ygrid,1)]);
     h = figure;
     surf(Xgrid,Ygrid,modelGridNew);
     hold on;
     scatter3(targetVal(:,1),targetVal(:,2),targetVal(:,3),30,[0 1 0],'filled');
     title('Target Output of Validation Data on Approximated Surface');
     saveas(h,'ApproxSurfaceVal.fig')
     colormap copper
     color bar
