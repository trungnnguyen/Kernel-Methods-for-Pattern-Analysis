...
class4_test=load('../../group6/class4_test.txt');
class4_train=load('../../group6/class4_train.txt');
class4_val=load('../../group6/class4_val.txt');

traindata=[class1_train;class2_train;class3_train;class4_train];
valdata=[class1_val;class2_val;class3_val;class4_val];
testdata=[class1_test;class2_test;class3_test;class4_test];

labelstrain=[ones(250,1)*[1,0,0,0];ones(250,1)*[0,1,0,0];ones(250,1)*[0,0,1,0];ones(250,1)*[0,0,0,1]];
labelstrain=vec2ind(labelstrain')';

labelsval=[ones(150,1)*[1,0,0,0];ones(150,1)*[0,1,0,0];ones(150,1)*[0,0,1,0];ones(150,1)*[0,0,0,1]];
labelsval=vec2ind(labelsval')';

labelstest=[ones(100,1)*[1,0,0,0];ones(100,1)*[0,1,0,0];ones(100,1)*[0,0,1,0];ones(100,1)*[0,0,0,1]];
labelstest=vec2ind(labelstest')';

totaldata=[class1_train;class1_val;class1_test;class2_train;class2_val;class2_test;class3_train;class3_val;class3_test;class4_train;class4_val;class4_test];
labelsdata=[ones(500,1)*[1,0,0,0];ones(500,1)*[0,1,0,0];ones(500,1)*[0,0,1,0];ones(500,1)*[0,0,0,1]];
labelsdata=vec2ind(labelsdata')';

model = svmtrain(labelstrain, traindata, '-s 1 -t 1 -d 3 -g 0.002 -r 1 -c 1 -n 0.5');
...
for j=1:D_SV
    ...
    if(labelstrain(SV(j))==4)
        h4=plot(traindata(SV(j),1),traindata(SV(j),2),'rs');
    end
end
...
xlabel('Feature 1');
ylabel('Feature 2');
saveas(H,'DB_Nu_poly_overlap.png');
close(H);
